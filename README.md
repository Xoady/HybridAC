...
### Сборка через Gradle (рекомендуется)
...
mvn clean package
```
Скомпилированный `.jar` будет создан в каталоге `target/hybridac-plugin-1.0.0.jar`.
...
│   │       ├── messages.yml        # Сообщения по умолчанию
...
- `messages.yml` / `messages/` — шаблоны сообщений и цветов оформления.
